...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadProfiles() {
        ...
        try {
            const profileNames = await apiRequest('/api/profiles');
    ...
    profileList.addEventListener('click', async (e) => {
        ...
        if (button.classList.contains('connect-btn')) {
            addLog(`正在连接到 ${alias}...`);
            try {
                await apiRequest('/api/session', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ alias }) });
        ...
        if (button.classList.contains('edit-btn')) {
            ...
            try {
                const profileData = await apiRequest(`/api/profiles/${alias}`);
        ...
        if (button.classList.contains('delete-btn')) {
            ...
            addLog(`正在删除账号: ${alias}...`);
            try {
                await apiRequest(`/api/profiles/${alias}`, { method: 'DELETE' });
                addLog('删除成功!', 'success');
                loadProfiles();
            } catch (error) {}
        }
    ...
    document.getElementById('saveProfileChangesBtn').addEventListener('click', async () => {
        ...
        try {
            const originalProfileData = await apiRequest(`/api/profiles/${originalAlias}`);
    ...
    async function performInstanceAction(action) {
        ...
        try {
            const response = await apiRequest('/api/instance-action', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
    ...
    async function loadSnatchTasks() {
        ...
        try {
            const [running, completed] = await Promise.all([
                apiRequest('/api/tasks/snatching/running'),
                apiRequest('/api/tasks/snatching/completed')
            ]);
    ...
    function pollTaskStatus(taskId) {
        ...
        const intervalId = setInterval(async () => {
            ...
            try {
                const result = await apiRequest(`/api/task_status/${taskId}`);
    ...
    if (document.getElementById('submitCreateInstance')) {
        document.getElementById('submitCreateInstance').addEventListener('click', async () => {
            ...
            try {
                const response = await apiRequest('/api/create-instance', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(details) });
                addLog(response.message, 'success');
    ...
    document.getElementById('submitSnatchInstanceBtn').addEventListener('click', async () => {
        ...
        try {
            const response = await apiRequest('/api/snatch-instance', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(details) });
            addLog(response.message, 'success');
