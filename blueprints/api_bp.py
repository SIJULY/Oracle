...
def instance_action_for_alias(alias):
    ...
    return jsonify({"success": True, "message": f"Action '{action}' for instance '{instance_id}' has been queued.",
                    "task_id": task_id}), 202
...
def get_completed_snatch_tasks():
    try:
        tasks = query_db_api(
            "SELECT id, name, status, result, created_at, account_alias FROM tasks WHERE type = 'snatch' AND (status = 'success' OR status = 'failure') ORDER BY created_at DESC LIMIT 50")
        return jsonify([dict(task) for task in tasks])
    except Exception as e:
        return jsonify({"error": str(e)}), 500
