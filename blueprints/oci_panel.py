...
def get_running_snatching_tasks():
    try:
        tasks = query_db \
            ("SELECT id, name, result, created_at, account_alias, status FROM tasks WHERE type = 'snatch' AND status IN ('running', 'paused') ORDER BY created_at DESC")
        tasks_list = []
        for task in tasks:
            task_dict = dict(task)
            try:
...
def get_completed_snatching_tasks():
    tasks = query_db \
        ("SELECT id, name, status, result, created_at, completed_at, account_alias FROM tasks WHERE type = 'snatch' AND (status = 'success' OR status = 'failure') ORDER BY created_at DESC LIMIT 50")
...
def resume_tasks():
    ...
    for task_id in task_ids:
        ...
        if not profile_config:
            failed_tasks.append(task_id)
            _db_execute_celery("UPDATE tasks SET status = ?, result = ?, completed_at = ? WHERE id = ?",
            ('failure', '任务因关联的账号配置被删除而恢复失败。', datetime.datetime.now(timezone.utc).isoformat(),
            task_id))
            continue
        ...
        try:
            ...
            _db_execute_celery('UPDATE tasks SET status = ?, result = ? WHERE id = ?',
                               ('running', json.dumps(result_json), task_id))
            ...
            failed_tasks.append(task_id)
            _db_execute_celery('UPDATE tasks SET status = ?, result = ?, completed_at = ? WHERE id = ?', (
            'failure', f'手动恢复任务失败: {e}', datetime.datetime.now(timezone.utc).isoformat(), task_id))
...
def get_instances(alias):
    try:
        ...
        for instance in instances:
            data = {"display_name": instance.display_name, "id": instance.id,
                    "lifecycle_state": instance.lifecycle_state, "shape": instance.shape,
                    ...
                    "memory_in_gbs": getattr(instance.shape_config, 'memory_in_gbs', 'N/A'), "public_ip": "无",
                    "ipv6_address": "无", "boot_volume_size_gb": "N/A", "vnic_id": None, "subnet_id": None}
            try:
                if instance.lifecycle_state not in ['TERMINATED', 'TERMINATING']:
                    ...
                    boot_vol_attachments = oci.pagination.list_call_get_all_results(
                        compute_client.list_boot_volume_attachments, instance.availability_domain, compartment_id,
                        instance_id=instance.id).data
                ...
                if se.status == 404:
                    logging.warning(
                        f"Could not fetch details for instance {instance.display_name} ({instance.id}), it might have been terminated. Error: {se.message}")
                else:
...
def _create_task_entry(task_type, task_name, alias=None):
    ...
    db.execute(
        'INSERT INTO tasks (id, type, name, status, result, created_at, account_alias) VALUES (?, ?, ?, ?, ?, ?, ?)',
...
def get_instance_details(instance_id):
    try:
        ...
        boot_vol_attachments = oci.pagination.list_call_get_all_results(compute_client.list_boot_volume_attachments,
                                                                        instance.availability_domain, compartment_id,
                                                                        instance_id=instance.id).data
...
def get_available_shapes():
    try:
        ...
        for shape in all_shapes:
            if shape.shape.startswith('VM.') and hasattr(shape,
                                                         'processor_description') and shape.processor_description:
        ...
        logging.info(
            f"Found {len(valid_shapes_for_os)} shapes compatible with {os_name_version}: {valid_shapes_for_os}")
...
def update_security_rules():
    try:
        ...
        vnet_client = g.oci_clients['vnet']
        update_details = UpdateSecurityListDetails(ingress_security_rules=rules.get('ingress_security_rules', []),
...
def _update_instance_details_task(task_id, profile_config, data):
    ...
    try:
        ...
        if action == 'update_display_name':
            ...
            shape_config = UpdateInstanceShapeConfigDetails(ocpus=data.get('ocpus'),
    ...
    try:
        ...
        if action_upper in action_map:
            ...
            _db_execute_celery('UPDATE tasks SET result=? WHERE id=?',
                               (f'等待实例进入 {target_state} 状态...', task_id))
            oci.wait_until(compute_client, compute_client.get_instance(instance_id), 'lifecycle_state', target_state,
            ...
            oci.wait_until(compute_client, compute_client.get_instance(instance_id), 'lifecycle_state', 'TERMINATED',
                           max_wait_seconds=300, succeed_on_not_found=True)
            ...
            new_pub_ip = vnet_client.create_public_ip(
                CreatePublicIpDetails(compartment_id=profile_config['tenancy'], lifetime="EPHEMERAL",
                                      private_ip_id=primary_private_ip.id)).data
...
def _snatch_instance_task(task_id, profile_config, alias, details, run_id, auto_bind_domain=False):
    ...
    _db_execute_celery('UPDATE tasks SET status = ?, result = ? WHERE id = ?',
                       ('running', json.dumps(status_data), task_id))
    ...
    try:
        ...
        base_launch_details = {
            ...
            "source_details": InstanceSourceViaImageDetails(image_id=images[0].id,
                                                            boot_volume_size_in_gbs=details['boot_volume_size']),
            "shape_config": LaunchInstanceShapeConfigDetails(ocpus=details.get('ocpus'), memory_in_gbs=details.get(
        ...
        _db_execute_celery('UPDATE tasks SET status = ?, result = ?, completed_at = ? WHERE id = ?', (
        'failure', f"❌ 抢占任务准备阶段失败: {e}", datetime.datetime.now(timezone.utc).isoformat(), task_id))
        return
    ...
    while True:
        ...
        try:
            ...
            _db_execute_celery('UPDATE tasks SET status = ?, result = ?, completed_at = ? WHERE id = ?', (
            'failure', "任务数据损坏，无法继续执行。", datetime.datetime.now(timezone.utc).isoformat(), task_id))
            return
        ...
        try:
            ...
            status_data[
                'last_message'] = f"第 {status_data['attempt_count']} 次尝试成功！实例 {instance.display_name} 正在置备..."
            _db_execute_celery('UPDATE tasks SET result = ? WHERE id = ?', (json.dumps(status_data), task_id))
            ...
            _db_execute_celery('UPDATE tasks SET status = ?, result = ?, completed_at = ? WHERE id = ?',
                               ('success', db_msg, datetime.datetime.now(timezone.utc).isoformat(), task_id))
            ...
            force_update = True
            if e.status == 429 or "TooManyRequests" in e.code or "Out of host capacity" in str(
                    e.message) or "LimitExceeded" in e.code:
